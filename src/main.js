                ...
                <div class="max-w-[400px] min-h-[230px] flex flex-col p-6 bg-white rounded-xl gap-4 shadow-sm hover:shadow-lg transition-shadow" id="${set.id}" data-type="set">
                    ...
                    <div class="w-[75%] flex items-center gap-6 mb-5">
                        <div class="flex items-center gap-1">
                            ...
                            <p class="text-gray-500">${setCard.length} tarjetas</p>
                        </div>
    
                        <div class="flex items-center gap-1">
                            <svg class="icon" width="16px" height="16px">
                                <use xlink:href="./assets/sprite-Dy07MmdU.svg#icon-calendar"></use>
                            </svg>
                            <p class="text-gray-500">${set.date}</p>
                        </div>
                    </div>
                    <button class="w-full ${setCard.length ? "bg-blue-500" : "bg-gray-300"} rounded-xl p-4 flex items-center justify-center text-white font-bold gap-4 ${setCard.length ? "cursor-pointer" : "cursor-not-allowed"} ${setCard.length ? "hover:scale-[1.01]" : ""} transition-transform" data-type="study-button" ${setCard.length === 0 ? "disabled" : ""}>${setCard.length ? "Estudiar" : "Sin tarjetas"}</button>
                </div>
            `
        }).join("");
    };
